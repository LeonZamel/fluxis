...
from .serializers import CredentialsSerializer, OAuth2CredentialsSerializer, DatabaseCredentialsSerializer
from .services.database.database_providers import DATABASE_PROVIDERS_CHOICE
from .services.oauth2.oauth2_providers import (OAUTH2_PROVIDERS,
                                               OAUTH2_PROVIDERS_CHOICE)
...
class DatabaseServiceList(APIView):
    def get(self, request):
        return Response(data=[{'key': key, 'name': name} for (key, name) in DATABASE_PROVIDERS_CHOICE])


class OAuth2ServiceList(APIView):
    def get(self, request):
        return Response(data=[{'key': key, 'name': name} for (key, name) in OAUTH2_PROVIDERS_CHOICE])
...
class OAuth2Start(APIView):
    def get(self, request, format=None):
        try:
            ...
            return HttpResponseBadRequest("No valid service key provided")
        ...
        response = Response(data={'url': authorization_url, })
...
class OAuth2Callback(APIView):
    def post(self, request):
        ...
        oauth = OAuth2Session(
            provider.client_id, redirect_uri=provider.redirect_uri, state=request.session['oauth_state'])
        token = oauth.fetch_token(provider.token_url, client_secret=provider.client_secret,
                                  authorization_response=resp)

        OAuth2Credentials.objects.create(
            user=request.user, access_token=token['access_token'], refresh_token=token.get('refresh_token'), token=token, service=service_key)
